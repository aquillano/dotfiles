...
local sizeup = { }
local log = hs.logger.new('sizeup', 'debug')

----------------------
-- Defaults/Options --
----------------------
sizeup.show_alerts = false
...
function sizeup.send_window_left_third()
  local s = sizeup.screen()
  local ssp = sizeup.third_screen_partitions
  local g = sizeup.gutter()
...
function sizeup.send_window_middle_third()
  local s = sizeup.screen()
  local ssp = sizeup.third_screen_partitions
  local g = sizeup.gutter()
  sizeup.set_frame("Middle Third", {
    x = s.x + (s.w * ssp.x) + g.x,
    y = s.y,
    w = (s.w * ssp.x) - g.x,
    h = s.h
...
function sizeup.send_window_right_third()
  ...
  local ssp = sizeup.third_screen_partitions
  local g = sizeup.gutter()
  local width = (s.w * ssp.x) - sizeup.gutter().x
  sizeup.set_frame("Middle Third", {
    x = s.x + (2 * width),
    y = s.y,
    w = width,
...
function sizeup.send_window_left_two_thirds()
  local s = sizeup.screen()
  local ssp = sizeup.two_third_screen_partitions
  local g = sizeup.gutter()
  sizeup.set_frame("Left Two Thirds", {
    x = s.x,
    y = s.y,
    w = (s.w * ssp.x) - sizeup.gutter().x,
    h = s.h
